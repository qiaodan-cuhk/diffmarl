...
# TBD: 
# 1. 检查逻辑错误，如果zip(obs_prey)只有一个参数，i就是obs而不是下标，导致self.preys[i]报错
# 2. JAL重复创建self.agents，可以精简
# 3. 考虑简化代码
...
from .SRPO_GMM import SRPO_GMM, SRPO_GMM_CTDE, SRPO_GMM_ssd
...
class BASE_SRPO_GMM(object):
    def __init__(
        ...
        **kwargs
        ...
        self.agents = [SRPO_GMM(input_dim = self.state_dim+self.action_dim, output_dim=self.action_dim, marginal_prob_std=marginal_prob_std_fn, args=config) for agent in alg_types]
    ...
    def prep_training(self, device='cpu'):
        ...
        for a in self.agents:
            ...
            a.gmm_score_model = fn(a.gmm_score_model)
...
class IND_SRPO_GMM(BASE_SRPO_GMM):
    def __init__(
        self, 
        agent_init_params, # state & action dim
        agent_max_actions, 
...
class JAL_SRPO_GMM(BASE_SRPO_GMM):
    def __init__(
        self, 
        agent_init_params, # state & action dim
        agent_max_actions, 
        ...
        **kwargs)
        ...
        self.agents = [SRPO_GMM(input_dim = self.state_dim+self.action_dim, output_dim=self.action_dim, marginal_prob_std=marginal_prob_std_fn, args=config)]
    ...
    def step(self, observations, explore=False):
        ...
        if self.env_id in ['simple_world', 'simple_tag']:
            ...
            """
            注意检查逻辑错误，如果zip(obs_prey)只有一个参数，i就是obs而不是下标，导致self.preys[i]报错
            for idx, obs in enumerate(obs_prey): 可以考虑这个
            """
...
class CTDE_SRPO_GMM(BASE_SRPO_GMM):
    def __init__(
        self, 
        agent_init_params, # state & action dim
        agent_max_actions, 
        ...
        **kwargs)
        ...
        self.agents = [SRPO_GMM_CTDE(input_dim = self.state_dim+self.action_dim,
    ...
    def update(self, samples, t, writer, run):
        ...
        for agent_id, current_agent in enumerate(self.agents):
            ...
            current_agent.gmm_score_model.eval()
        ...
        **kwargs)
        ...
        for i in range(self.nagents):
            ...
            if stage_idx == 0:
                ...
                self.agents.append(SRPO_GMM_CTDE(
                    input_dim=self.state_dim + self.action_dim,
                    output_dim=self.action_dim,
                    marginal_prob_std=marginal_prob_std_fn,
                    args=config
                ...
                self.agents.append(SRPO_GMM_ssd(
                    input_dim=input_dim,
                    output_dim=self.action_dim,
                    marginal_prob_std=marginal_prob_std_fn,
