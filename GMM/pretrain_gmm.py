"""Pretrain conditional GMM behavior p(a|obs): IND / CTDE / JAL / Seq (MASRPO_Behavior_GMM)."""
# train ind/jal 的逻辑没有更新，暂时先不要用
...
from GMM.SRPO_GMM import MASRPO_Behavior_GMM, gmm_nll_loss
from torch.utils.tensorboard import SummaryWriter


# conditional_order sanity check
...
def train_ind_behavior(args, score_model, data_loader, agent_num, writer, start_epoch=0):
    ...
    for epoch in tqdm_epoch:
        ...
        writer.add_scalar("agent {}/lr".format(agent_num), score_model.gmm_optimizer.state_dict()['param_groups'][0]['lr'], epoch+1)
        ...
        if args.save_model and epoch_loss < best_loss:
            ...
            torch.save(score_model.state_dict(), os.path.join("/data/qiaodan/code/diffmarl/GMM/pretrain_gmm", f"{args.env_id}_{args.data_type}", "IND", "best_gmm_{}.pth".format(agent_num)))
            # SRPO_premodels/env_id_level/IND/best_gmm_i.pth
        
        if args.save_model and epoch % epoch_save_interval == (epoch_save_interval - 1): 
            print("Save models: Epoch {}".format(epoch))
            torch.save(score_model.state_dict(), os.path.join("/data/qiaodan/code/diffmarl/GMM/pretrain_gmm", f"{args.env_id}_{args.data_type}", "IND", "gmm_{}_epoch{}.pth".format(agent_num, epoch)))
            # SRPO_premodels/env_id_level/IND/gmm_i_epoch150.pth   
...
def train_joint_behavior(args, score_model, data_loader, writer, start_epoch=0):
    ...
    for epoch in tqdm_epoch:
        ...
        epoch_loss = score_model.loss.detach().cpu().numpy()
        writer.add_scalar("JAL/lr", score_model.gmm_optimizer.state_dict()['param_groups'][0]['lr'], epoch+1)
        ...
        if args.save_model and epoch_loss < best_loss:
            best_loss = epoch_loss
            print("New lowest loss in epoch {}, Save best models".format(epoch))
            torch.save(score_model.state_dict(), os.path.join("/data/qiaodan/code/diffmarl/GMM/pretrain_gmm", f"{args.env_id}_{args.data_type}", "JAL", "best_gmm.pth"))
            # SRPO_premodels/env_id_level/JAL/best_gmm.pth
        
        if args.save_model and epoch % epoch_save_interval == (epoch_save_interval - 1): 
            print("Save models: Epoch {}".format(epoch))
            torch.save(score_model.state_dict(), os.path.join("/data/qiaodan/code/diffmarl/GMM/pretrain_gmm", f"{args.env_id}_{args.data_type}", "JAL", "gmm_{}.pth".format(epoch)))
            # SRPO_premodels/env_id_level/JAL/gmm_150.pth
...
def train_seq_behavior(args, score_model, data_loader, agent_num, writer, start_epoch=0):
    ...
    for epoch in tqdm_epoch:
        ...
        if args.save_model and epoch % epoch_save_interval == (epoch_save_interval - 1): 
            print("Save models: Epoch {}".format(epoch))
            torch.save(score_model.state_dict(), os.path.join(args.save_dir, "gmm_agent{}_epoch{}.pth".format(agent_num, epoch)))
            # .../Seq/<conditional_order>/gmm_agent_i_epochN.pth   
 

        
# Pretrain GMM behavior
def behavior(args):
    # make env
    ...
    if args.srpo_mode == 'IND' or args.srpo_mode == 'CTDE':
        score_model= [MASRPO_Behavior_GMM(input_dim=state_dim+action_dim, output_dim=action_dim, marginal_prob_std=marginal_prob_std_fn, args=args).to(args.device) for agent in range(agent_num)]
    elif args.srpo_mode == 'JAL':
        score_model= MASRPO_Behavior_GMM(input_dim=(state_dim+action_dim)*agent_num, output_dim=action_dim*agent_num, marginal_prob_std=marginal_prob_std_fn, args=args).to(args.device)
        # JAL model is p(all_a|all_obs)
    elif args.srpo_mode == 'Seq':
        # 支持指定的扰动顺序
        ensure_conditional_order_list(args)
        if args.seq_agent_id not in args.conditional_order_list:
            raise ValueError(f"agent {args.seq_agent_id} is not in conditional_order {args.conditional_order_list}")

        agent_order_idx = args.conditional_order_list.index(args.seq_agent_id)

        score_model = MASRPO_Behavior_GMM(
                            input_dim=state_dim + action_dim + (agent_order_idx * action_dim if agent_order_idx > 0 else 0), # 这里需要更新确认
    ...
    if args.env_id in ['HalfCheetah-v2', 'Ant-v2', 'Hopper-v2']:
        ...
        raise ValueError(f"Env {args.env_id} not supported")
    ...
    tb_log_path = os.path.join("/data/qiaodan/code/diffmarl/ablation_gmm_Logs",
    ...
    """ Model Saving Dir """
    args.save_dir = os.path.join(args.save_dir, f"{args.n_gmm_components}_GMM", f"{args.env_id}_{args.data_type}_{args.srpo_mode}")
    if not os.path.exists(args.save_dir):
        os.makedirs(args.save_dir)
    print(f"Ckpt save path: {args.save_dir}")

    print("Start training GMM behavior")
    ...
    print("Finished training GMM behavior")
...
def pretrain_behavior_args():
    ...
    parser.add_argument("--dataset_num", default=0, type=int, help="Dataset seed number from 0-4, used for MPE Tasks")
    ...
    # params for GMM
    parser.add_argument("--n_gmm_components", default=8, type=int, help="GMM mixture size K (MASRPO_Behavior_GMM)")
    ...
    parser.add_argument("--save_dir", default="/data/qiaodan/code/diffmarl/pretrain/gmm", type=str)
    ...
    # combine dir for omiga datasets
    ...
    # print config
    print(f"Training GMM behavior {config.env_id}: {config.data_type}: Dataset Num {config.dataset_num}: Agent {config.seq_agent_id}")
