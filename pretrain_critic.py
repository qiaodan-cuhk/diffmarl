...
from algorithms.SRPO import SRPO_IQL
from utils import get_args, pallaral_simple_eval_policy


def train_joint_critic(args, score_model, data_loader, start_epoch=0):
    n_epochs = 150
...
def train_ind_critic(args, score_model, data_loader, start_epoch=0):
    n_epochs = 150
    tqdm_epoch = tqdm.trange(start_epoch, n_epochs)
    # evaluation_inerval = 4
    evaluation_inerval = 1
...
def critic(args):
    ...
    if args.diffusion_mode == 'CTDE' or 'IND':
        train_joint_critic(args, score_model, dataset, start_epoch=0)
    elif args.diffusion_mode == 'JAL':
        train_ind_critic(args, score_model, dataset, start_epoch=0)
    print("finished")
