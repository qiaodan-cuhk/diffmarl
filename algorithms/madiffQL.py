...
from utils.ql_diffusion import Diffusion_QL
...
class MADiff(object):
    def __init__(
        ...
        **kwargs
        ...
        # params 包含 state_dim, action_dim
        ...
        for i, obs in zip(range(self.nagents), observations):   
            if self.env_id in ['simple_world', 'simple_tag']:
                if i < self.num_predators:
                    predator_action = self.agents[i].sample_action(obs)
                ...
                action = self.agents[i].sample_action(obs)
    ...
    def update(self, sample, agent_i, t):
        ...
        metric = curr_agent.train(sample, batch_size=self.batch_size, log_writer=None, is_mujoco = self.is_mamujoco)
        
        """ logging metric """
        if t % self.logging_interval == 0 and not self.no_log:
            dic = {}
            dic.update({"BC loss agent"+str(agent_i):metric['bc_loss']})
            dic.update({"QK loss agent"+str(agent_i):metric['ql_loss']})
            dic.update({"Actor loss agent"+str(agent_i):metric['actor_loss']})
            dic.update({"Critic loss agent"+str(agent_i):metric['critic_loss']})
